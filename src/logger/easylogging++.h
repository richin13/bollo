...
/// @brief String utilities helper class used internally. You should not use it.
        ...
        replaceFirstWithEscape(str, replaceWhat, base::type::string_t(replaceWith.begin(), replaceWith.end()));
                ...
                static inline char* convertAndAddToBuff(std::size_t n, int len, char* buf, const char* bufLim,
            ...
            tv->tv_usec = static_cast<long>(present % usecOffSet);
                ...
                static inline std::string getDateTime(const char* format, const base::MillisecondsWidth* msWidth) {
                    ...
                    parseFormat(buff_, kBuffSize, format, &timeInfo,
                                static_cast<std::size_t>(currTime.tv_usec / msWidth->m_offset), msWidth);
                ...
                static inline unsigned long long getTimeDifference(const struct timeval& endTime,
                        ...
                        if(*format == base::consts::kFormatSpecifierChar) {
                            switch(*++format) {
                                ...
                                case 'b':  // month (short)
                                    ...
                                    continue;
                                case 'B':  // month (long)
                                    buf = base::utils::Str::addToBuff(base::consts::kMonths[tInfo->tm_mon], buf,
                                                                      bufLim);
                                    continue;
...
/// @detail This repository represents configurations for all the levels and configuration type mapped to a value.
    ...
    private:
        ...
        void unsafeSetIfNotExist(Level level, ConfigurationType configurationType, const std::string& value) {
            Configuration* conf = RegistryWithPred<Configuration, Configuration::Predicate>::get(level,
                                                                                                 configurationType);
            if(conf == nullptr) {
...
/// This is thread safe and final class containing non-virtual destructor (means nothing should inherit this class)
        ...
        private:
            ...
            template<typename Conf_T>
            inline Conf_T unsafeGetConfigByVal(Level level, const std::map<Level, Conf_T>* confMap,
                                               const char* confName) {
                ...
                for(Configurations::const_iterator it = configurations->begin(); it != configurations->end(); ++it) {
                    ...
                    if(conf->configurationType() == ConfigurationType::Enabled) {
                        ...
                        setValue(Level::Global,
                                 base::MillisecondsWidth(static_cast<int>(getULong(conf->value()))),
            ...
            unsigned long getULong(std::string confVal) {
                ...
                valid = !confVal.empty() && std::find_if(confVal.begin(), confVal.end(),
                                                         [](char c) { return !base::utils::Str::isDigit(c); }) ==
                                            confVal.end();
                if(!valid) {
                    valid = false;
            ...
            void insertFile(Level level, const std::string& fullFilename) {
                ...
                auto create = [&](Level level) {
                    base::LogStreamsReferenceMap::iterator filestreamIter = m_logStreamsReference->find(
                            resolvedFilename);
            ...
            bool unsafeValidateFileRolling(Level level, const PreRollOutCallback& PreRollOutCallback) {
                ...
                if(maxLogFileSize != 0 && currFileSize >= maxLogFileSize) {
                    ...
                    ELPP_INTERNAL_INFO(1,
                                       "Truncating log file [" << fname << "] as a result of configurations for level ["
                                       << LevelHelper::convertToString(level) << "]");
                    fs->close();
    ...
    private:
        ...
        void resolveLoggerFormatSpec(void) const {
            ...
            LevelHelper::forEachLevel(&lIndex, [&](void) -> bool {
                ...
                base::utils::Str::replaceFirstWithEscape(logFormat->m_format, base::consts::kLoggerIdFormatSpecifier,
                                                         m_id);
                return false;
            });
        }
...
/// @brief Represents registries for verbose logging
        ...
        public:
            explicit VRegistry(base::type::VerboseLevel level, base::type::EnumType* pFlags) : m_level(level),
                                                                                               m_pFlags(pFlags) {
            }
...
/// @brief Easylogging++ management storage
        ...
        base::utils::safeDelete(m_asyncLogQueue);
            ...
            inline bool validateAfterNCounter(const char* filename, unsigned long int lineNumber,
                                              std::size_t n) { // NOLINT
                return hitCounters()->validateAfterN(filename, lineNumber, n);
            }

            inline bool validateNTimesCounter(const char* filename, unsigned long int lineNumber,
                                              std::size_t n) { // NOLINT
        ...
        return m_asyncLogQueue;
            ...
            inline bool installPerformanceTrackingCallback(const std::string& id) {
                return installCallback<T, base::type::PerformanceTrackingCallbackPtr>(id,
        ...
        public:
            base::type::string_t build(const LogMessage* logMessage, bool appendNewLine) const {
                ...
                if(logFormat->hasFlag(base::FormatFlags::File)) {
                    ...
                    base::utils::Str::replaceFirstWithEscape(logLine, base::consts::kLogFileFormatSpecifier,
                                                             std::string(buff));
                }
                ...
                if(logFormat->hasFlag(base::FormatFlags::Line)) {
                    ...
                    base::utils::Str::replaceFirstWithEscape(logLine, base::consts::kLogLineFormatSpecifier,
                                                             std::string(buff));
                }
...
#   define ELPP_SIMPLE_LOG(LOG_TYPE)\
    inline MessageBuilder& operator<<(LOG_TYPE msg) {\
        ...
        return *this;\
            ...
            ELPP_SIMPLE_LOG(signed
                                    short)

            ELPP_SIMPLE_LOG(unsigned
                                    short)
...
/// @brief Main entry point of each logging
        ...
        protected:
            ...
            void initializeLogger(const std::string& loggerId, bool lookup = true, bool needLock = true) {
                if(lookup) {
                    m_logger = ELPP->registeredLoggers()->get(loggerId,
...
#   define ELPP_CURR_FILE_PERFORMANCE_LOGGER el::base::consts::kPerformanceLoggerId
    ...
    public:
        ...
        explicit PerformanceTrackingData(DataType dataType) : m_performanceTracker(nullptr),
                                                              m_dataType(dataType), m_file(""), m_line(0),
                                                              m_func("") { }
...
///        either when goes outside the scope of when checkpoint() is called
        ...
        public:
            PerformanceTracker(const std::string& blockName,
                    ...
                    m_blockName(blockName), m_timestampUnit(timestampUnit), m_loggerId(loggerId),
            ...
            void checkpoint(const std::string& id = std::string(), const char* file = __FILE__,
                            unsigned long int line = __LINE__, const char* func = "") {
#if !defined(ELPP_DISABLE_PERFORMANCE_TRACKING) && ELPP_LOGGING_ENABLED
                if(m_enabled) {
                    base::threading::ScopedLock scopedLock(lock());
                    base::utils::DateTime::gettimeofday(&m_endTime);
        ...
        protected:
            void handle(const PerformanceTrackingData* data) {
                ...
                el::base::Writer(m_data->performanceTracker()->level(), m_data->file(), m_data->line(),
                                 m_data->func()).construct(1, m_data->loggerId().c_str()) << ss.str();
            }
...
/// @brief Contains some internal debugging tools like crash handler and stack tracer
        namespace debug {
            ...
            public:
                ...
                public:
                    StackTraceEntry(std::size_t index, const char* loc, const char* demang, const char* hex,
                                    const char* addr) {
                        m_index = index;
                    ...
                    friend std::ostream& operator<<(std::ostream& ss, const StackTraceEntry& si) {
                        ss << "[" << si.m_index << "] " << si.m_location << (si.m_demangled.empty() ? "" : ":") <<
                        si.m_demangled
